  ...
  <div>
    <form @submit.prevent="updateCars">
      <div class="form-group">
        <h2 class="heading">Update Car Registration</h2>
        <div class="controls">
          <label for="number">Car Name</label>
          <input type="text" id="name" v-model="car.carname"  class="floatLabel" name="name" />
        </div>
        <div class="controls">
          <label for="number">Registration Number</label>
          <input type="number" id="name" v-model="car.registrationNo"  class="floatLabel" name="name" />
        </div>
        <div class="controls">
          <label for="number">Modal</label>
          <input type="number" id="name" v-model="car.modalNo"  class="floatLabel" name="modal" />
        </div>
        <div class="controls">
          <label for="price">Price</label>
          <input type="number" id="price" v-model="car.price"  class="floatLabel" name="price" />
        </div>
        <div class="controls">
          <label for="available">category</label>
          <select class="floatLabel col-1-xl" required v-model="car.Category" name="category">
...
import { mapActions } from "vuex";
  export default {
    ...
    async mounted(){
        const response =await  axios.get('/car/'+this.$route.params.id)
        this.car = response.data
        console.log("id update",this.$route.params.id)
    },
    methods: {
     ...mapActions(["addCar"]),
      updateCars() {
